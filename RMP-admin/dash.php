    ...
    if (!empty($sessionvalue) && !empty($termvalue)) {
        ...
        if (mysqli_num_rows($classQuery) > 0) {
            ...
            if (mysqli_num_rows($student_query) > 0) {
                ...
                echo '<div class="panel title" style="margin-top: 20px;">';
                echo '<img src="uploads/logo.jpg" alt="Dominican Logo" style="width: 100px; height: auto; display: block; margin: 0 auto;">';
                echo "<h3 style='text-align: center; color: navy;'>DOMINICAN COLLEGE MAFOLUKU</h3>";
                ...
                foreach ($students_data as $sid => &$student_data) {
                    ...
                    foreach ($subjects as $subject_id => $subject_name) {
                        ...
                        $ft_mark = $row['total_mark'] ?? null; // Use null if no marks are found
                        if ($ft_mark !== null) {
                        ...
                        $result = mysqli_query($con, $query);
                        $row = mysqli_fetch_assoc($result);
                        $st_mark = $row['total_mark'] ?? null; // Use null if no marks are found
                        if ($st_mark !== null) {
                        ...
                        $result = mysqli_query($con, $query);
                        $row = mysqli_fetch_assoc($result);
                        $tt_mark = $row['total_mark'] ?? null; // Use null if no marks are found
                        if ($tt_mark !== null) {
                ...
                echo '<button id="download-btn" style="padding: 10px 20px; margin: 20px 0; background-color: navy; color: white; border: none; cursor: pointer;">Download Cumulative Broadsheet</button>';
                echo '</div>';
...
document.getElementById('download-btn').addEventListener('click', function() {
    ...
    var opt = {
        margin:       0.5,
        filename:     'cumulative_broadsheet.pdf',
