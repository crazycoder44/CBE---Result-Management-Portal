...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$noinclassresult || mysqli_num_rows($noinclassresult) === 0) {
        echo "No data found";
        exit;
    }
    ...
    foreach ($ca_marks as &$marks) {
        ...
        $insert_query = "INSERT INTO $temp_table_name (email, totalscore, position)
                         ...
                         WHERE sub_id = '$sub_id' AND termid = '$termvalue' AND session = '$sessionvalue'
                           AND email IN (SELECT email FROM results WHERE classid LIKE '$baseclassid%')
    ...
    foreach ($ca_marks as &$marks) {
        ...
        $insert_query = "INSERT INTO $temp_table_name (email, totalscore, position)
                         ...
                         FROM results
                         WHERE sub_id = '$sub_id' AND termid = '$termvalue' AND session = '$sessionvalue'
                           AND email IN (SELECT email FROM results WHERE classid = '$class')
                         ORDER BY totalscore DESC";
        mysqli_query($con, $insert_query);
